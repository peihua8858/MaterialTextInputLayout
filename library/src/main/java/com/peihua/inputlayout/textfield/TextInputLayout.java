...
package com.peihua.inputlayout.textfield;
...
import com.peihua.inputlayout.R;
import com.peihua.inputlayout.internal.CollapsingTextHelper;
import com.peihua.inputlayout.shape.CornerFamily;
import com.peihua.inputlayout.shape.CornerTreatment;
import com.peihua.inputlayout.shape.MaterialShapeDrawable;
import com.peihua.inputlayout.shape.ShapeAppearanceModel;
...
/**
 * Layout which wraps a {@link TextInputEditText}, {@link EditText}, or descendant to
 ...
 *       also specify an {@link OnClickListener}, an {@link
...
public class TextInputLayout extends LinearLayout {
    ...
    public void setCounterEnabled(boolean enabled) {
        if (counterEnabled != enabled) {
            if (enabled) {
                ...
                indicatorViewController.addIndicator(counterView, IndicatorViewController.COUNTER_INDEX);
                ...
                indicatorViewController.removeIndicator(counterView, IndicatorViewController.COUNTER_INDEX);
                counterView = null;
            }
    ...
    private Rect calculateCollapsedTextBounds(@NonNull Rect rect) {
        ...
        switch (boxBackgroundMode) {
            case BOX_BACKGROUND_OUTLINE:
                ...
                if (hideHintCutout) {
                    bounds.top = rect.top + boxCollapsedPaddingTopPx - calculateLabelMarginTop();
                } else {
     ...
     *
     * @param startIconOnClickListener the {@link OnClickListener} the start icon
     ...
     *
     * @param startIconOnLongClickListener the {@link OnLongClickListener} the start
     ...
     *
     * @param endIconOnClickListener the {@link OnClickListener} the end icon view
     *                               will have
     */
     ...
     *
     * @param errorIconOnClickListener the {@link OnClickListener} the error icon
     *                                 view will have
     */
     ...
     * will not be clickable if its click and long click listeners are null.
     *
     * @param endIconOnLongClickListener the {@link OnLongClickListener} the end
     *                                   icon view will have, or null to clear it.
     */
     ...
     * icon will not be clickable if its click and long click listeners are null.
     *
     * @param errorIconOnLongClickListener the {@link OnLongClickListener} the error
     *                                     icon view will have, or null to clear it.
     */
     ...
     * EndIconMode} via {@link #setEndIconMode(int)}.
     *
     * <p>If you use an icon you should also set a description for its action using {@link
    ...
    /**
     * Add a {@link OnEndIconChangedListener} that will be invoked when the end icon
     * gets changed.
     *
    ...
    /**
     * Remove the given {@link OnEndIconChangedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link OnEndIconChangedListener}s.
     */
    ...
    /**
     * Sets an {@link AccessibilityDelegate} providing an accessibility implementation
...
//                && (endLayout.getMeasuredWidth() > 0);
    ...
    public void setFixedExpand(boolean isExpand) {
        isFixedExpand = isExpand;
        hintExpanded = isExpand;
        collapsingTextHelper.setExpansionFraction(isExpand ? 1f : 0f);
        startLayout.onHintStateChanged(isExpand);
        endLayout.onHintStateChanged(isExpand);
    }
    ...
    public void addEndLayout(View view, LayoutParams params) {
        endLayout.setCustomView(view, params);
    }

    public void setStartLayout(View view, LayoutParams params) {
        startLayout.setCustomView(view, params);
    }
}
